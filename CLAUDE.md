...
| `@rxc/compat-controls` | `packages/compat-controls` | Legacy compat for `@react-typed-forms/core` consumers — **complete (Phases A+B+C)**: compat `Control<V>` type, `ControlImpl.prototype` patch (zero overrides), ambient read collector + `withAmbient`, ambient write transactions (`groupedChanges`), singleton context, full function surface, `useComponentTracking`/`useTrackedComponent` (SWC-plugin contract), all legacy hooks, F-components, render helpers, `formControlProps`/`useFormControlProps`, `trackedValue`/`unsafeRestoreControl`/`unwrapTrackedControl`, `SubscriptionTracker`/`Effect`/`AsyncEffect` + factories. Migration = alias/rename the import + one root `<ControlContextProvider value={getCompatContext()}>`. Design: `docs/COMPAT-CONTROLS-DESIGN.md`. |
...
The `@rxc/*` packages haven't been published. Don't add `@deprecated` aliases, "legacy" re-exports, backwards-compat shims, or rename-with-pointer transitions when refactoring the public surface. Just change the name / shape and update every call site. The dedicated `@rxc/compat-controls` package exists for legacy `@react-typed-forms/core` consumers — that's the only place compat shims belong. (Legacy `@react-typed-forms/schemas` consumers port directly via `docs/MIGRATION-FROM-LEGACY.md` — a schemas compat package was considered and rejected.)
