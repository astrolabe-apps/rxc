...
| `rxc-forms-v2-poc` | `poc/forms-v2` | **Throwaway.** Vite app that builds the `docs/FORMS-V2-INTERFACES.md` contract four ways over — plain HTML, MUI, Ant, Mantine — with field, options, collection, group, action, display and dialog boundaries, a tab container, a wizard, a staged-edit modal, third-party field, collection, action and group renderers, and a JSON loader that returns what it could not translate. `rushx extract-corpus` pulls a legacy app's forms and schemas into a gitignored `corpus/`; `rushx burndown` runs the loader over it and reports by kind and shape — that number is goal 6's acceptance test and the loader's work list (README, *Where to pick up*). Its README carries the findings; the docs carry what they changed. Port 5183. Owes the rest of the repo nothing and is expected to be deleted. |
...
- Current counts:
  ...
  - **Total: 622** across the 7 packages that have tests. Re-check with `rush test`, or
