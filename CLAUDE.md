...
| `@rxc/forms-react-core` | `packages/forms-react-core` | Headless React forms layer: registry, matchers, dispatch helpers, adornment composition, plugin builders, contexts (Registry/Options/ActionScope/DesignMode), and hooks (useFormStateNode/useLabelText/useExpression/useAsyncAction/useExternalEdit/useFormErrors). Also exports `<Action>` (the only "component" — a one-liner over `pickActionRenderer`). No DOM-emitting components — platform packages provide those. |
...
| `@rxc/forms-datagrid` | `packages/forms-datagrid` | Optional DataGrid add-on. Ships `dataGridRegistry()` — `DataGrid` + `Pager` data renderers + `ColumnOptions` adornment, layered on the published `@astroapps/datagrid` base grid. Columns + rows from the bound array, column **filter/sort** header controls (driven by a sibling `SearchOptions` control via `searchField`), offset/length **paging**, per-column `visible`/`rowSpan` expressions, adjacent-key `groupByField` row-spanning, and **add/remove/edit** array actions including the **editExternal** modal-staged flow (via `useExternalEdit`, hosted by the renderer itself — DataGrid is self-contained, not the sibling-control pattern Array uses). `clientSearchPage`/`fieldClientSearch` helpers for client-side search (filters/sort/query). All buttons (Add/Edit/Remove/Apply/Cancel + Pager prev/next) render via `<Action>` so hosts can override chrome per id via `matchActionId`. |
...
- Current counts:
  ...
  - `forms-react-core`: **57** (+10 useExternalEdit suite including sibling-FormStateNode controller sharing)
...
- **`@rxc/forms-react-core`: `useExternalEdit` + the sibling `ArrayElementModalHostRenderer` pattern.** Ported the legacy `@react-typed-forms/schemas` "Add / Edit stages a draft, modal commits on Apply" flow for arrays. Two pieces:
  - **`useExternalEdit(arrayNode, options?)`** — returns a per-array controller exposing `session(rc)`, `beginAdd(initialValue?)`, `beginEdit(index)`, `apply({ dontValidate? })`, `cancel()`. Session is a standalone draft `Control` + standalone `FormStateNode` rooted on the array's element schema/form; `apply()` validates via `draftForm.validate()` (touching all nodes on failure so errors surface) and writes through `wc.addElement` / `wc.setValue(elem, draft)`. **The controller is cached on `arrayControl.meta["$externalEdit"]`** — not on the FormStateNode meta — so two sibling FormStateNodes bound to the same array field share **one** controller and **one** session. The shared cache is the linchpin: `Array` writes a session, the sibling modal host reads from the same cache. `staticDef` arg added to `createFormStateNode` so a custom draft definition (e.g. DataGrid's `Contents` group over columns) can shadow the form node's own definition for the root.
  - **`ArrayElementModalHostRenderer`** (`@rxc/forms`) — new renderer for `renderType: ArrayElement` *bound to an array itself* (not an element; `elementIndex === undefined` + `field.collection === true`). Renders **nothing** until a session exists, then pops a native `<dialog>` with the draft form + Apply/Cancel `<Action>` buttons. The two-sibling form-definition shape is:
    ```ts
    ...
    ```
    `<Field>` dispatches each control to its own renderer; both resolve to the same array `Control`, so they share the session. `ArrayRenderer` + per-row `ArrayElementRenderer` *do not* host their own modal — only the sibling host does. `DataGrid` is the exception: it hosts its modal internally (single self-contained renderer; no sibling required). Reproduces the legacy `@react-typed-forms/schemas-html@5.2.1` shape; the legacy package shipped a fix during this work to wire the previously-stubbed modal body via `createChildNode("draft", ...) + getResolvedChildren()`.
...
- **`ArrayElementRenderer`** — `DataRenderType.ArrayElement` on an **element** (matched via `DataCursor.elementIndex !== undefined`). Renders a one-line summary + Edit button. When the parent array has `editExternal: true`, the Edit button dispatches `editController.beginEdit(index)` and the local `<dialog>` is **not** rendered (the sibling `ArrayElementModalHostRenderer` displays the staged draft). When `editExternal` is unset, Edit opens a local `<dialog>` showing the live element's children (legacy compact-view UX, mutations commit immediately). `showInline: true` skips both paths and renders children inline. `renderType: ArrayElement` *on the array itself* (elementIndex undefined) routes to `ArrayElementModalHostRenderer` — see "Upstream changes" above.
...
- **`noReorder` / drag-to-reorder**: rxc has no built-in row reordering. The `@rxc/forms-dnd` `SortableArrayRenderer` is the closest analogue but isn't wired into the grid; a future port could combine the two.
