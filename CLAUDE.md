...
- Current counts:
  - `controls-core`: **65** (uniqueId determinism; +9 `controlGroup`/`setFields`: value/initial composition, bidirectional sync, multi-parent sharing, validity aggregation, field swap/detach, no-op detection)
  ...
  - **Total: 488** across the 7 packages that have tests. Re-check with `rush test`, or
