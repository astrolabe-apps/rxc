...
| `@rxc/forms-datagrid` | `packages/forms-datagrid` | Optional DataGrid add-on. Ships `dataGridRegistry()` — `DataGrid` + `Pager` data renderers + `ColumnOptions` adornment, layered on the published `@astroapps/datagrid` base grid. Columns + rows from the bound array, column **filter/sort** header controls (driven by a sibling `SearchOptions` control via `searchField`), offset/length **paging**, per-column `visible`/`rowSpan` expressions, adjacent-key `groupByField` row-spanning, and **add/remove/edit** array actions (legacy `editExternal` modal not ported — host-driven via `<ActionScope>`). `clientSearchPage`/`fieldClientSearch` helpers for client-side search (filters/sort/query). |
...
- Current counts:
  ...
  - `forms-datagrid`: **18** (3 display-only resolver + 9 clientSearch filter/sort/paging/query + 3 computeGroupRowSpans + 3 getDataGridLengthRange)
...
- **Array add/remove/edit actions**: `addText`/`removeText`/`editText` + `addActionId`/`removeActionId`/`editActionId` added to `DataGridOptions`. Renderer reads `min`/`max` via `getDataGridLengthRange(def)` (`Length` validator, with `required` defaulting `min` to 1) and disables buttons at the boundaries. Each button dispatches via `useActionHandler` first; if no `<ActionScope>` claims the action, falls back to direct `wc.addElement`/`wc.removeElement` mutation. `editActionId` has no built-in fallback — hosts must provide an `<ActionScope>` handler to open a modal/route/etc. (the legacy `editExternal` modal flow is intentionally not ported). Buttons hidden in `displayOnly` mode and when the cascade reports `readonly`/`disabled`. Add button below the grid; remove/edit in the trailing `cdeleteCheck` cell. Button class composition matches `ButtonAction`/`Pager` (action theme's `buttonLayoutClass` + `buttonClass` + `primaryClass`).
...
- **External-edit dialog flow** (`editExternal` / `getExternalEditData`): the legacy "Add" / "Edit" modal that stages a draft row and only commits on Apply. Hosts that want this in rxc should drive it themselves via `<ActionScope>` intercepting `add`/`edit` action ids. Not on the immediate roadmap.
