...
The argument: a form should be React first, and the JSON the editor produces should become
something we load onto that, not the thing we build in. I'll show you why, show you the
...
<div class="cols two">
  ...
  <div><h3 class="bad">Where it hurts</h3><ul>
    <li>Developers building forms in a tool made for non-developers: no types, no refactor, no tests</li>
...
<div class="demo-card">
  <p class="what">What you're about to see</p>
  <ol>
    <li>The <b>Legacy form</b> tab loads <code>formDefs/MastSummary.json</code> from ServiceTas, unchanged, through <code>&lt;JsonForm&gt;</code>: four sections, nested groups, text fields, checkboxes, arrays.</li>
...
Back to the browser. Legacy form tab. MastSummary is selected: sixty-three controls, the marine
licences and registrations data, four sections with nested groups and arrays. Switch to MUI, then Ant.

Then tick the warnings box. Five items for this form. Read one or two aloud: they're specific,
...
If time allows, pick Fire or Address from the dropdown. If a form looks rough, say why: the classes
