...
<li>Open the app, click the <b>Legacy form</b> tab once. Address should render with "6 things the loader could not translate". Then back to <b>Details</b>, implementation <b>html</b>, click <b>Reset</b>.</li>
...
<p>If you're behind at 16:30, drop slide 14 (the Stars source): slide 12 already made the claim and the demo already showed it. If you're behind at 24:30, do Demo 2 as Address only, skip the warnings box.</p>
...
<ol class="steps">
<li>Click the <b>Legacy form</b> tab. <b>Address · 38</b> is selected. Read the grey line: <code>formDefs/Address.json</code>, "6 things the loader could not translate".</li>
<div class="say">"This is the ServiceTas Address form, byte for byte what's in the repo. It went through the loader and came out as the same components you saw on the other tabs. Under MUI, because that's what's selected."</div>
...
<div class="say">"Six for this form. Over the whole corpus, 792, and nineteen forms have none. That number is the work list, and it's the acceptance test for goal six."</div>
<li>If ahead: <b>Legacy form → Fire · 35</b>. The materials radio list with descriptions renders. The broken image icons are icon-display translation gaps; say so.</li>
...
<div class="warn">Avoid <b>ContactUs</b> live: its icon-list actions render as large blue blocks (the icon names become button text). Address and Fire are the tested pair. The ServiceTas Tailwind classes (<code>body</code>, <code>title1</code>) come through on the elements but the POC ships no stylesheet for them, which is why spacing is plain. Say it before someone asks.</div>
